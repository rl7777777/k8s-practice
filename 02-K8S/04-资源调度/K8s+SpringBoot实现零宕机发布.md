...
K8s + SpringBoot实现零宕机发布：健康检查+滚动更新+优雅停机+弹性伸缩+Prometheus监控+配置分离（生产环境镜像复用）
...
若业务层面支持健康检查，设置`strategy`为`RollingUpdate`即可
...
若为配置健康检查，在之前的基础上需声明`minReadySeconds`和`progressDeadlineSeconds`字段
...
spec:
  ...
  strategy:
    ...
    rollingUpdate:
      maxSurge: 1                   # 升级过程中最多可以比原先设置的副本数多出的数量
      maxUnavailable: 1             # 升级过程中最多有多少个POD处于无法提供服务的状态
  minReadySeconds: 60               # 被认为是可用状态的最小秒数，默认0，即等待指定时间后才转为可用状态
  progressDeadlineSeconds: 120      # 升级最大时间
```
...
spec:
  ...
  - type: Resource
    resource:
      name: cpu                        # 指定资源指标
      targetAverageUtilization: 75     # 根据资源利用率计算缩放比例
```
            ...
            podAffinityTerm:
              labelSelector:
                ...
                - key: app
                  ...
                  values:
                  - {APP_NAME}
              topologyKey: "kubernetes.io/hostname" # aws为"topology.kubernetes.io/zone"
